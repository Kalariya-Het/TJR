...
export const UserManagement: React.FC = () => {
  ...
  const handleToggleStatus = async (userId: string, currentStatus: boolean) => {
    ...
    try {
      ...
      const data = await response.json();
      if (response.ok) {
        setSuccessMessage(`User ${data.user.email} status toggled successfully.`);
        fetchUsers();
      } else {
  ...
  const handleUpdateKycStatus = async (e: React.FormEvent) => {
    ...
    setSuccessMessage(null);
    try {
      const response = await fetch(`http://localhost:3001/api/admin/users/${selectedUser.id}/kyc`, {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`,
    ...
    return (
      <div className="text-center py-12">
        ...
        <p className="text-gray-500">
          You need to be an admin to manage users.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-8">
      ...
      <p className="text-gray-600">Manage all users in the system, including their status and KYC verification.</p>
          ...
          <table className="min-w-full divide-y divide-gray-200">
            <thead className="bg-gray-50">
              <tr>
                ...
                <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">KYC</th>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                    <div className="flex gap-2">
                      ...
                      <Button
                        ...
                        size="sm"
                      >
                        <PencilIcon className="h-4 w-4 mr-1" /> KYC
                      </Button>
                    </div>
                  </td>
                </tr>
